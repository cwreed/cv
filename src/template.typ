...
#let cvEntry(
  ...
  table(
    ...
    table(
      ...
      {
        entryA2Style(
          ifSocietyFirst(varEntrySocietyFirst, location, date),
          accentColor,
        )
      },
      ...
      {
        entryB1Style(
          ifSocietyFirst(varEntrySocietyFirst, title, institution),
          accentColor,
        )
      },
...
#let cvFooter(pageNumbers: bool) = {
  ...
  if pageNumbers {
    left = left + [, page #context counter(page).display("1/1", both: true)]
    table(
      columns: (1fr, auto),
      inset: 0pt,
      stroke: none,
